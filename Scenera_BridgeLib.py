...
import json
from Scenera_GenerateNewSceneData import SceneDataValues, CreateSceneData
import time
import threading
from Scenera_ImageAndVideoClasses import EventClass, EventVideoClass
...
#strBrigdeUUID = "00000001-5cdd-280b-8002-0df860000ffd0" #DC's Bridge
...
def DeviceNodeID(strUUID, NodeID):
    hexNodeID = str(NodeID)
    #print("hexNodeID " , hexNodeID)
    hexNodeIDPadded =  hexNodeID.zfill(4)
    DeviceNodeID = strUUID + "_" + hexNodeIDPadded
    #print("DeviceNodeID ==", DeviceNodeID)
    return(DeviceNodeID)

def DeviceNodeID_INT(strUUID, NodeID):
    hexNodeID = str(NodeID)
    #print("hexNodeID " , hexNodeID)
    hexNodeIDPadded =  hexNodeID.zfill(4)
    DeviceNodeID = strUUID + "_" + hexNodeIDPadded
...
def DevicePortID(strUUID, NodeID, PortID):
    hexNodeID = str(NodeID)
    hexNodeIDPadded =  hexNodeID.zfill(4)
    hexPortID =  str(PortID)
    hexPortIDPadded =  hexPortID.zfill(4)
    DevicePortID = strUUID + "_" + hexNodeIDPadded + "_" + hexPortIDPadded
    return(DevicePortID)

def DevicePortID_INT(strUUID, NodeID, PortID):
    hexNodeID = str(NodeID)
    hexNodeIDPadded =  hexNodeID.zfill(4)
    hexPortID =  str(PortID)
    hexPortIDPadded =  hexPortID.zfill(4)
...
def CreateSceneMarkID(strBrigdeUUID,NodeID,Instance):
    #global NodeID
...
def CreateSceneDataID(strBrigdeUUID, NodeID,Instance):
    #global NodeID
    #global strBrigdeUUID
...
def GetSceneModeThread():
    ...
    while True:
        try:
            strDeviceNodeID = DeviceNodeID(strBrigdeUUID, NodeID)
            objGlobalGetSceneMode = GetSceneMode(API_ENDPOINT_CONTROLER, strDeviceNodeID, strBrigdeUUID, strCloudServerUUID, NodeID, PortID, strAccessTokenController)
